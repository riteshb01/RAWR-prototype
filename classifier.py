...
# Check sklearn availability once at import time
try:
...
BOOTSTRAP_TRAINING_DATA = [
    ...
    ("HW 4 due by Friday April 5", "assignment"),
    ("Assignment 2 submit before midnight", "assignment"),
    ("Midterm Exam - Week 8", "exam"),
    ("MIDTERM: October 15th in class", "exam"),
    ("Final Exam - December 18, 2026", "exam"),
    ("Comprehensive Final: May 2", "exam"),
...
class EventClassifier:
    ...
    EVENT_LABELS = ['none', 'assignment', 'quiz', 'project',
                    'presentation', 'exam', 'lab', 'reading']
    ...
    def _try_load_model(self):
        ...
        if not SKLEARN_AVAILABLE:
            logger.warning("scikit-learn not installed — ML classifier unavailable")
            return
        if MODEL_PATH.exists() and VECTORIZER_PATH.exists():
            try:
                ...
                if self.model is not None and self.feature_names is not None:
                    ...
                    raise ValueError("Loaded model or feature_names is None")
            except Exception as e:
                logger.warning(f"Could not load ML model: {e}. Will retrain.")
                self.model = None
                self.feature_names = None
                self.is_trained = False
                # Remove stale files
    ...
    def retrain_from_db(self):
        ...
        try:
            ...
            training_data = [(e['raw_line'], e['event_type']) for e in db_events if e.get('raw_line')]

            if training_data:
                all_data = BOOTSTRAP_TRAINING_DATA + training_data
                return self.train(all_data)
            else:
                logger.info("No verified events in DB — using bootstrap data only")
                return self.train()
        except ImportError:
            logger.warning("Django models not available — skipping DB retraining")
        except Exception as e:
...
def get_classifier() -> EventClassifier:
    ...
    if _classifier_instance is None:
        ...
        if not _classifier_instance.is_trained:
            success = _classifier_instance.train()  # Bootstrap from default data
            if not success:
                logger.warning("Classifier training failed — returning untrained instance")
